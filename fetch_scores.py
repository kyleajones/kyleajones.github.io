...
def fetch_scores():
    ...
    for game in data:
        # We only want games marked as completed that have actual score data
        if game.get('completed') and game.get('scores'):
            scores_list = game['scores']
            
            # Convert the scores list into a simple dictionary: {"Chiefs": 27, "Ravens": 20}
            team_scores = {
                team['name']: int(team['score']) 
                for team in scores_list if team.get('score') is not None
            }
            
            # Use the game.id as the master key for instant JavaScript lookups
    ...
    print(f"Successfully saved {len(completed_games)} completed games to results.json.")
